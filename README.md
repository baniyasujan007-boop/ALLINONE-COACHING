...
Full-stack Coaching App built with Flutter, Node.js, Express, and MongoDB. Includes student and admin modules, course and quiz management, payment tracking, community features, secure JWT authentication, file uploads, Google sign-in, and AI-powered doubt solving.
...
For ChatGPT, set OpenAI values in `backend/.env`:
...
2. Run the app with API URL override:
...
`/api/ai/doubt-solve` supports ChatGPT via OpenAI and can also accept an image with the doubt.
...
- Protected routes with `protect` and `adminOnly`
- Input validation with `express-validator`
- Centralized error handling middleware

## Notes

- If the backend fails with `EADDRINUSE: 5001`, another process is already using port `5001`. Change `PORT` in `backend/.env` or stop the existing process.
